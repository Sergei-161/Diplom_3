...
def login_user(driver, create_new_user):
    ...
    # Ждём, что действительно попали на страницу логина
    login_page.wait_url_contains(URLS.url_login)

    assert login_page.is_auth_form_visible(), "Форма авторизации не загрузилась"

    login_page.login(user_data["email"], user_data["password"])

    # Ждём редирект на главную страницу
    login_page.wait_url_to_be(MainUrl.MAIN_URL)

    current_url = login_page.get_current_url()
    assert current_url == MainUrl.MAIN_URL, (
        f"Логин не прошел успешно. Текущий URL: {current_url}"
    )
